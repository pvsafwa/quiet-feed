...
export function VideoCard({ v, queue, onPress }: { v: Video; queue?: Video[]; onPress?: () => void }) {
  ...
  const handlePress = () => {
    if (onPress) onPress();
    else openPlayer(v, queue);
  };
  return (
    <motion.article className="card" variants={itemV} onClick={handlePress} whileHover={{ y: -3 }} style={{ cursor: 'pointer' }}>
